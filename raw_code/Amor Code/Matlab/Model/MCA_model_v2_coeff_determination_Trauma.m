function MCA_model_v2_coeff_determination_Trauma
...
fitrightrealpole0 = ones(5,1);
%[fitrightrealpolefactors,resfitrightrealpolenorm] = lsqcurvefit(@fitrightrealpoleModel,fitrightrealpole0,[II_0 VIII_0 ATIII_0 V_0],rightrealpolevalue',[],[],options)
...
[fitrightrealpolefactors,resfitrightrealpolenorm] = lsqcurvefit(@fitrightrealpoleModel,fitrightrealpole0,[ATIII_0 X_0 VII_0 V_0],rightrealpolevalue',[],[],options)

%rightrealpolepredict = fitrightrealpolefactors(5)*V_0 + fitrightrealpolefactors(4)*ATIII_0 + fitrightrealpolefactors(3)*VIII_0 + fitrightrealpolefactors(2)*II_0 + fitrightrealpolefactors(1);
...
rightrealpolepredict = fitrightrealpolefactors(5)*V_0 + fitrightrealpolefactors(4)*VII_0 + fitrightrealpolefactors(3)*X_0 + fitrightrealpolefactors(2)*ATIII_0 + fitrightrealpolefactors(1);

figure(4)
%plot(II_0,rightrealpolevalue,'k*',II_0,rightrealpolepredict,'bo')
%plot(V_0,rightrealpolevalue,'k*',V_0,rightrealpolepredict,'bo')
plot(ATIII_0,rightrealpolevalue,'k*',ATIII_0,rightrealpolepredict,'bo')
%xlabel('II_0')
%xlabel('V_0')
xlabel('ATIII_0')
...
[fitleftpairmagpolefactors,resfitleftpairmagpolenorm] = lsqcurvefit(@fitleftpairmagpoleModel,fitleftpairmagpole0,[X_0 IX_0 VII_0 VIII_0 II_0],leftpairmagpolevalue',[],[],options)
...
leftpairmagpolepredict = fitleftpairmagpolefactors(6)*II_0 + fitleftpairmagpolefactors(5)*VIII_0 + fitleftpairmagpolefactors(4)*VII_0 + fitleftpairmagpolefactors(3)*IX_0 + fitleftpairmagpolefactors(2)*X_0 + fitleftpairmagpolefactors(1);
...
plot(X_0,leftpairmagpolevalue,'k*',X_0,leftpairmagpolepredict,'bo')
%xlabel('ATIII_0')
xlabel('X_0')
...
[fitkdfactors,resfitkdnorm] = lsqcurvefit(@fitkdModel,fitkd0,[X_0 V_0 IX_0 VII_0],kd,[],[],options)

kdpredict = fitkdfactors(5)*VII_0 + fitkdfactors(4)*IX_0 + fitkdfactors(3)*V_0 + fitkdfactors(2)*X_0 + fitkdfactors(1);
...
plot(X_0,kd,'k*',X_0,kdpredict,'bo')
%xlabel('II_0')
xlabel('X_0')
ylabel('k_d')
grid on
...
fitrightrealpoleEstimate = x(5)*V_Init + x(4)*VII_Init + x(3)*X_Init + x(2)*ATIII_Init + x(1);
...
fitleftpairmagpoleEstimate = x(6)*II_Init + x(5)*VIII_Init + x(4)*VII_Init + x(3)*IX_Init + x(2)*X_Init + x(1); 
...
fitkdEstimate = x(5)*VII_Init + x(4)*IX_Init + x(3)*V_Init + x(2)*X_Init + x(1);

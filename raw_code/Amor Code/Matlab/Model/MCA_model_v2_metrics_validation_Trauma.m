function MCA_model_v2_metrics_validation_Trauma
...
%VisualNum = 15; %1--40...Good: 15!, 18!, 30!, 39!
%25 is hemorrhagic but predicted thrombotic
...
[MaxMeasuredThrombin,IndexMaxMeasuredThrombin] = max(MeasuredThrombin);
TimeatMaxMeasuredThrombin = Tcheck(IndexMaxMeasuredThrombin);
ETPMeasuredThrombin = trapz(Tcheck,MeasuredThrombin);
...
PredictedSystemElapsedTime(VisualNum) = toc;
...
[MaxPredictedThrombin,IndexMaxPredictedThrombin] = max(Y);
TimeatMaxPredictedThrombin = T(IndexMaxPredictedThrombin);
ETPPredictedThrombin = trapz(T,Y);

AbsPercentErrorofPeak(VisualNum) = 100*abs(MaxPredictedThrombin - MaxMeasuredThrombin)/MaxMeasuredThrombin;
AbsPercentErrorTimeofPeak(VisualNum) = 100*abs(TimeatMaxPredictedThrombin - TimeatMaxMeasuredThrombin)/TimeatMaxMeasuredThrombin;
AbsPercentErrorETP(VisualNum) = 100*abs(ETPPredictedThrombin - ETPMeasuredThrombin)/ETPMeasuredThrombin;
RMSofPercentErrors(VisualNum,1) = sqrt(AbsPercentErrorofPeak(VisualNum)^2 + AbsPercentErrorTimeofPeak(VisualNum)^2 + AbsPercentErrorETP(VisualNum)^2);
RMSofPercentErrors(VisualNum,2) = VisualNum;

end

%AbsPercentErrorofPeak
%AbsPercentErrorTimeofPeak
%RMSofPercentErrors

MeanPredictedSystemElapsedTime = mean(PredictedSystemElapsedTime)

MeanAbsPercentErrorofPeak = mean(AbsPercentErrorofPeak)
MeanAbsPercentErrorTimeofPeak = mean(AbsPercentErrorTimeofPeak)

SortedRMSPercentErrors = sortrows(RMSofPercentErrors,1)


figure(1)
clf
subplot(2,2,1)
hist(AbsPercentErrorofPeak,5:10:195)
xlabel('{\itP} Percent Error')
ylabel('Frequency')
grid on
box on
...
subplot(2,2,2)
hist(AbsPercentErrorTimeofPeak,5:10:55)
xlabel('{\itT_P} Percent Error')
ylabel('Frequency')
grid on
box on

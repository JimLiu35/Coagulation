...
% WB_Fit_Par=TEG_WB_Fit_Parameters([1:2,5:6,8:13,18:24],:) ; %15
% WB_Factors=TEG_WB_Factor_Concentration([1:2,5:6,8:13,18:24],:) ;
% TEG_exp = TEG_WB_experiment_data(:,[1:2,5:6,8:13,18:24]) ;

% WB_Fit_Par=TEG_WB_Fit_Parameters([1:9,11,15,16,19,20,22,24],:) ; 
% WB_Factors=TEG_WB_Factor_Concentration([1:9,11,15,16,19,20,22,24],:) ;
% TEG_exp = TEG_WB_experiment_data(:,[1:9,11,15,16,19,20,22,24]) ;
...
TEG_WB_Validation_Exp=xlsread('TEG_WB_Plasma',1,'O2:S722') ;
TEG_WB_Validation_Exp=TEG_WB_Validation_Exp(:,[2 1 5 3 4]); %correcting the order of samples 
TEG_WB_Validation_Exp_Time=xlsread('TEG_WB_Plasma',1,'N2:N722')./60 ;
TEG_WB_Validation_CoagFact=xlsread('Cohen Lab prelim data',1,'E2:P6') ;
...
% %Kp2
% md1=fitlm(WB_Factors(:,10),WB_Fit_Par(:,4));
% Regression_weight(1:2,4)=md1.Coefficients.Estimate;
% eval1=[ones(length(WB_Factors),1),WB_Factors(:,10)]*Regression_weight(1:2,4);
% R4=RSquaredValue(WB_Fit_Par(:,4),eval1)
% 
% %Kn2
% md1=fitlm(WB_Factors(:,10),WB_Fit_Par(:,5));
% Regression_weight(1:2,5)=md1.Coefficients.Estimate;
% eval1=[ones(length(WB_Factors),1),WB_Factors(:,10)]*Regression_weight(1:2,5);
% R5=RSquaredValue(WB_Fit_Par(:,5),eval1)
% 
% %Kn2/Kp2
% md1=fitlm(WB_Factors(:,10),WB_Fit_Par(:,5)./WB_Fit_Par(:,4));
% Regression_weight(1:2,6)=md1.Coefficients.Estimate;
% eval1=[ones(length(WB_Factors),1),WB_Factors(:,10)]*Regression_weight(1:2,6);
% R6=RSquaredValue(WB_Fit_Par(:,5)./WB_Fit_Par(:,4),eval1)
...
% Validation_CoagFact=[TEG_Plasma_Validation_CoagFact(:,2:7),TEG_Plasma_Validation_CoagFact(:,10:12)]; 
...
%     Y_val_peak=Y_est_TEG(91);
%     Y_exp_peak=Y_exp(361,1);

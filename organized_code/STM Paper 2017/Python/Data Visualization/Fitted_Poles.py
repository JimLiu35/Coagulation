...
AverageRightRealPoleValue = np.mean(rightrealpolevalue)
AverageLeftPairMagPoleValue = np.mean(leftpairmagpolevalue)
AverageLeftPairAnglePoleValue = np.mean(leftpairanglepolevalue)
AverageDelay = np.mean(kd)

## Trauma Patients
# Read in trauma concentrations
CATT = '../../Data/Processed/CAT_Trauma.xlsx'
dfT = pd.read_excel(CATT, sheet_name='Fits', usecols="B:M").dropna()
MCADataValues = dfT[:40].to_numpy()
